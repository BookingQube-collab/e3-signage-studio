...
- During download the TV shows **Downloading… %**, file count, and stall/retry copy — never a blank navy frame with no feedback. **0.30+** keeps that waiting UI over the preparing player until ExoPlayer/image actually paints the first frame (early ACTIVE no longer flashes blank).
...
Install the versioned APK copied to the repo `dist/` folder (gitignored), e.g. `dist/e3-signage-player-0.30.0-debug.apk`. Open **E3 Signage**, enter the 6-digit code in the CMS **Pair a screen** dialog. Uninstall a previous sideload first if the TV keeps the old generic icon.
