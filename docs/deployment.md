...
| `CLOUD_STORAGE_QUOTA_BYTES` | Server | Optional org media library quota (bytes) for dashboard Storage low alerts (fires at used ≥ quota). Per-org override: `organization_settings.cloud_storage_quota_bytes` (default 8 GiB) |
