...
`assembleDebug` (and `assembleRelease`) also copy the APK into the repo `dist/` folder as `e3-signage-player-<versionName>-<buildType>.apk` (currently **0.27.0**). `dist/` is gitignored.
...
Current player version: **0.27.0** (`versionCode` 27).
...
2. Sideload via USB, a network share, or:
   ...
   ```
   adb connect <tv-ip>
   adb install -r dist/e3-signage-player-0.27.0-debug.apk
   ```
