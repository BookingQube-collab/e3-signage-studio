...
`assembleDebug` (and `assembleRelease`) also copy the APK into the repo `dist/` folder as `e3-signage-player-<versionName>-<buildType>.apk` (currently **0.30.0**). `dist/` is gitignored.
...
Current player version: **0.30.0** (`versionCode` 30).
...
2. Sideload via USB, a network share, or:
   ...
   ```
   adb connect <tv-ip>
   adb install -r dist/e3-signage-player-0.30.0-debug.apk
   ```

3. Open **E3 Signage** from Apps. Enter the 6-digit pairing code in the CMS **Pair a screen** dialog.
4. Within a few seconds the CMS screen detail **Heartbeat** should leave **Never** and the badge should flip to **Online**. If Heartbeat stays **Never**, the TV did not finish activation — use **Repair → Disconnect player**, wait for the TV pairing code (or long-press OK on the TV), then **Link new code**. Do not rely on **Sync Now**; that only flags a pull when the player already has a token.
...
1. In CMS **Screens**, open the screen menu → **Repair**.
2. Click **Disconnect player** (revokes the device token). Within ~15 seconds the player should leave playback and show a new 6-digit code.
